...
import { VirtualProperties } from "../../../src/components";
import { VirtualPropertiesProps } from "../../../src/hooks";
...
describe("VirtualProperties.vue", () => {
  ...
  it("should respect passed options to transformation", () => {
    wrapper = mountVirtualProperties(ogma, {
      ...
      enabled: true,
      options: {
        nodeSelector: node => node.getId() === 2,
        nodeDataFunction: () => ({ test: 1 }),
      }
    });
    return ogma.transformations.afterNextUpdate()
      .then(() => {
        ...
        expect(ogma.getNodes().getData())
          .toEqual([undefined, undefined, { test: 1 }, undefined]);
        wrapper.setProps({
          options: {
      ...
      .then(() => {
        expect(ogma.getNodes().getData())
          .toEqual([{ test: 1 }, undefined, undefined, undefined]);
