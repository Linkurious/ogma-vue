...
describe("Ogma.vue", () => {
  ...
  it("should register ogma events and trigger them", () => {
    ...
    return ogma.view.afterNextFrame().then(() => {
      ...
      return expect(wrapper.emitted()).to.have.property("addEdges");
    });
  });
  it("should trigger only specified events", () => {
    ...
    return ogma.view.afterNextFrame().then(() => {
      expect(wrapper.emitted()).to.have.property("addNodes");
      return expect(wrapper.emitted()).to.not.have.property("addEdges");
    });
  });
  it("should be reactive to graph change", () => {
    ...
    return nextTick().then(() => {
      ...
      return expect(ogma.getEdges().size).toBe(2);
    });
  });
  it("Should destroy on unmount", () => {
    ...
    wrapper.unmount();
    nextTick().then(() => {
      return expect(ogma.isDestroyed()).toBe(true);
    });
  });
});
